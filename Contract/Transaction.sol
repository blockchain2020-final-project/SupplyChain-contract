...
contract Transaction18 {
    ...
    struct Receipt {
        ...
        int256 amount;
        int256 deadline;
        int256 valid; // 0 - 待审核，1 - 接受，2 - 不接受, 3 - 还款阶段
    }
    ...
    // 信用凭证交易接受事件
    event TransactionEnd(address debtor, address debtee, int256 receiptId);
    ...
    constructor(address _adminAddr, string _suffix) public {
        adminAddr = _adminAddr;
        suffix = _suffix;

        TableFactory tf = TableFactory(0x1001);
        tf.createTable(
            concat("t_company", suffix),
            "addr",
            "name,bank,gov,in_balance,out_balance"
        );
        tf.createTable(
            concat("t_in_receipt", suffix),
            "debtee",
            "debtor,debtorName,debtee,debteeName,receiptId,amount,deadline,valid"
        );
        tf.createTable(
            concat("t_out_receipt", suffix),
            "debtor",
            "debtor,debtorName,debtee,debteeName,receiptId,amount,deadline,valid"
        );
    ...
    function addBalance(int256 amount) public {
        ...
        int256 inBalance;
        int256 outBalance;
        (inBalance, outBalance) = findCompanyBalance(msg.sender); // 债务人
        inBalance += amount;
    ...
    function registerBank(address addr, string name) public {
        require(
            ...
            "Only the Central Bank is allowed to banks registration"
        );
    ...
    function registerCompany(address addr, string name) public {
        ...
        require(
            company.gov,
            "Only governments are allowed to companies registration"
        );
    ...
    function registerGovernment(address addr, string name) public {
        require(
            msg.sender == adminAddr,
            "Only the Central Bank is allowed to governments registration"
        );
    ...
    function insertCompany(
        ...
        entry.set("name", name);
        entry.set("bank", bank ? int256(1) : int256(0));
        entry.set("gov", gov ? int256(1) : int256(0));
    ...
    function findCompanyBalance(address addr) private returns (int256, int256) {
        findCompany(addr);
    ...
    function transferCredit(
        ...
        int256 debtorIn;
        int256 debtorOut;
        (debtorIn, debtorOut) = findCompanyBalance(msg.sender); // 债务人
        int256 debteeIn;
        int256 debteeOut;
        (debteeIn, debteeOut) = findCompanyBalance(debtee); // 债权人
        require(
            debtorIn - debtorOut >= amount,
    ...
    function acceptTransferCredit(int256 receiptId, int256 valid) public {
        require(
            valid == 1 || valid == 2,
            "Only accepting or declining are allowed"
        );
        ...
        if (valid == 1) {
            ...
            int256 debteeIn;
            int256 debteeOut;
            (debteeIn, debteeOut) = findCompanyBalance(receipt.debtee); // 债权人
    ...
    function returnCredit(int256 receiptId, int256 amount) public {
        findReceipt("t_in_receipt", toString(msg.sender), receiptId);
        int256 debtorIn;
        int256 debtorOut;
        (debtorIn, debtorOut) = findCompanyBalance(receipt.debtor); // 债务人
        int256 debteeIn;
        int256 debteeOut;
        (debteeIn, debteeOut) = findCompanyBalance(receipt.debtee); // 债权人
        ...
        require(
            ...
            "Cannot return credit more than amount transfered at first"
        );
    ...
    function acceptReturnCredit(int256 receiptId, int256 valid) public {
        require(
            valid == 1 || valid == 2,
            "Only accepting or declining are allowed"
        );
        ...
        require(
            inReceipt.valid == outReceipt.valid,
            "Receipt states are nonconsistent"
        );
        require(
            inReceipt.valid == 3,
            "Only Receipts pending for returning are allowed to accept"
        );
        require(
            outReceipt.amount < inReceipt.amount,
            "Receipts states are nonconsistent"
        );
        ...
        int256 debteeOut;
        (debteeIn, debteeOut) = findCompanyBalance(receipt.debtee); // 债权人
        int256 amount = inReceipt.amount - outReceipt.amount;
